            ...
            <div class="well col-sm-12" style="background-color: #AED4F1; height:auto; margin:10px;">
                ...
                <div>
                    ...
                    if(isset($_POST['name']) && $_POST['name']=="TOTAL REGISTERED STUDENTS") {
                        ...
                        <input type='number' id='cgpaFilter' name='cgpa' step='0.01'></span><br>
                        <label for='backlogFilter'>Filter by Backlogs:</label>
                        <span style='margin-left: 20px;'>
                        <input type='number' id='backlogFilter' name='backlog'></span><br>
                        <label for='deptFilter'>Filter by Branch:</label>
                        <span style='margin-left: 40px;'>
                        <input type='text' id='deptFilter' name='branch'></span><br>
                    ...
                    if(isset($_POST['name']) && $_POST['name']=="TOTAL PLACED STUDENTS") {
                        ...
                        if ($result->num_rows > 0) {
                            ...
                            while($row = $result->fetch_assoc()) {
                                echo "<tr><td>".$row["name"]."</td><td>".$row["email"]."</td><td>".$row["phone"]."</td><td>".$row["degree"]."</td><td>".$row["branch"]."</td><td>".$row["cgpa"]."</td><td>".$row["twp"]."</td><td>".$row["tenp"]."</td><td>".$row["company_name"]."</td><td>".$row["job_title"]."</td><td>".$row["salary"]."</td
<td>".$row["salary"]."</td></tr>";
                            }
                            echo "</table>";
                        } else {
    ...
    <script>
        ...
        document.getElementById('exportBtn').addEventListener('click', function() {
            var selectedTable;
            var selectedOption = document.querySelector('select[name="name"]').value;
            
            if (selectedOption == "TOTAL REGISTERED STUDENTS") {
                selectedTable = document.getElementById('studentsTable');
            } else if (selectedOption == "TOTAL PLACED STUDENTS") {
                selectedTable = document.getElementById('placedStudentsTable');
            } else if (selectedOption == "TOTAL REGISTERED COMPANIES") {
                selectedTable = document.getElementById('companiesTable');
            } else if (selectedOption == "TOTAL VACANCIES") {
                selectedTable = document.getElementById('vacanciesTable');
            }
            
            var wb = XLSX.utils.table_to_book(selectedTable, { sheet: "Sheet JS" });
